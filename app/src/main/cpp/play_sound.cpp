...
int FMOD_Main()
{
    ...
    */
    ...
    result = system->createSound(Common_MediaPath("drumloop.wav"), FMOD_DEFAULT |FMOD_CREATESTREAM, 0, &sound1);
    ERRCHECK(result);
    result = sound1->setMode(FMOD_ACCURATETIME);
    ...
    */
    do
    {
        ...
        if (Common_BtnPress(BTN_ACTION1))
        {
            ...
            int yourValue =5;
            __android_log_print(ANDROID_LOG_INFO, "YourTag", "Value: %s", FMOD_ErrorString(result));
            ERRCHECK(result);*/
            FMOD_SOUND_TYPE type;
            FMOD_SOUND_FORMAT format;
            int channels;
            int bits;
            sound1->getFormat(&type, &format, &channels, &bits);
            __android_log_print(ANDROID_LOG_INFO, "YourTag", "Format: %s", FMOD_ErrorString(result));
            //LOGI("%d %d", type, format);

            sound1->getLength(&soundLength, FMOD_TIMEUNIT_PCMBYTES);
            __android_log_print(ANDROID_LOG_INFO, "YourTag", "Length: %s", FMOD_ErrorString(result));
            FMOD_RESULT result;

            const int CHUNK_SIZE = 4096;
            std::vector<char> myVector;
            unsigned int length = CHUNK_SIZE;
            while (length == CHUNK_SIZE) {
                std::vector<char> chunk(CHUNK_SIZE);

                    result = sound1->readData(chunk.data(), chunk.size(), &length);
                __android_log_print(ANDROID_LOG_INFO, "YourTag", "read Data: %s", FMOD_ErrorString(result));
                    //LOGI("read data %s", FMOD_ErrorString(result));

                //result = sound->readData(chunk.data(), chunk.size(), &length);
                //LOGI("read data %s", FMOD_ErrorString(result));
                myVector.insert(myVector.end(), chunk.begin(), chunk.begin() + length);
            }
            ...
            MyFile.write(myVector.data(), myVector.size());
            MyFile.close();
